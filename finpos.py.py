...
outputFinancialPosition = csv.reader(open("Financial Position.csv"))
outputIncomeStatement = csv.reader(open("Income Statement.csv"))
...
def financialSector():
        count = 0
        sector = raw_input("Which sector would you like to iterate through in Income Statement?: ")
        ...
        if ask == "help":
                ...
                for item in financial_response['rows']:
                        ...
                        if fp_response.get('sector', None) == sector and fp_response['company'].has_key(key):
                                count += 1
                                FinancialPositionCSV.writerow([
                                fp_response['company']['name'],
                                fp_response['company'][key]])
                                print count
                ...
                for item in financial_response['rows']:
                        ...
                        if fp_response.get('sector', None) == sector:
                                count += 1
                                FinancialPositionCSV.writerow([
                                fp_response['company']['name'],
...
def financial_position_fn():
    financialSector()
    sort = raw_input("Would you like to sort your data ascending or descending? If you don't want to sort it, type NO: ")
    if sort == "ascending":
            sortedlist = sorted(outputFinancialPosition, key=operator.itemgetter(1), reverse=True)
    elif sort == "descending":
            sortedlist = sorted(outputFinancialPosition, key=operator.itemgetter(1), reverse=False)
    elif sort == "no":
            print ("OK, my job is to offer... Geez, I just wanted to help...")
    print ("Done. Your data is stored in Financial Position.csv file. It contains '%s' entries.") % (financialSector().count)
